...
def put_piece(board, max_rows, column, player):
    ...
    """
    rightnow = max_rows - 1
    while rightnow >= 0 and get_piece(board, rightnow, column) != "-":
        rightnow = rightnow-1
    if rightnow >=0:
        newrow = replace_elem(board[rightnow], column, player)
        newboard = replace_elem(board, rightnow, newrow)
        board = newboard
    return rightnow, board
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    if col > max_cols or col < 0:
        return -1, board
    indexrow, newboard = put_piece(board, max_rows, col, player)
    return indexrow, newboard
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for col in range(max_cols):
        piece1 = get_piece(board, row, col)
        if piece1 == player:
            win_or_not = 1+ win_or_not
        else:
            win_or_not = 0
        if win_or_not >= num_connect:
            return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for row in range(max_cols):
        piece2 = get_piece(board, row, col)
        if piece2 == player:
            win_or_not = 1+ win_or_not
        else:
            win_or_not = 0
