...
class ComputerPlayer(Player):
    ...
    def guess(self, board, verbose=False):
        ...
        """
        # BEGIN
        lst = [a for a in Player.all_words if len(a) == len(board)]
        new_dict = {}
        for word in lst:
            for letter in word:
                new_dict[letter] = 0
        for word in lst:
            for letter in word:
                new_dict[letter] += 1
        while key_of_max(new_dict) in board.guesses():
            e = key_of_max(new_dict)
            new_dict[e] = 0
        return key_of_max(new_dict)
        # END

    def pick_word(self):
        """Pick a random word from the dictionary."""
        # BEGIN
        return random.sample(Player.all_words,1)
        # END
