...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        if type(person) != str:
            ...
            print("{0} says:".format(person),self.place.characters[person].talk())
    ...
    def take(self, thing):
        ...
        if type(thing) != str:
            ...
            itemsinside= self.place.take(thing)
            self.backpack.append(itemsinside)
    ...
    def unlock(self, place):
        ...
        for item in self.backpack:
            if type(item) == Key:
                key = item
        if key:
            unlockp = self.place.get_neighbor(place)
            key.use(unlockp)
        elif not key:
